 ...
 */
class  tx_itaoshhmanager_module1 extends t3lib_SCbase {
				var $pageinfo;
				...
				function main()	{
					...
					$this->doc->setModuleTemplate(t3lib_extMgm::extPath('itao_shh_manager') . 'mod1/mod_template.html');
          ...
          $has_pruefer_rights = 1;
				...
				function getBackLink($func_nr) {
					...
					if ($this->piVars['communeid']) {
						$backlink = '
							<a href="/typo3/mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM1">
...
/*'*/
				...
				function moduleContent()	{
					...
					*/
					...
					$this->indexpath = "/typo3/mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM1";		
					...
					switch((string)$this->MOD_SETTINGS['function'])	{
						case 1:
							...
							# Kommune reaktivieren:
							if ($this->piVars['reactcomm']) {
								tx_itaoshhmanager_database::finishCommune($this->piVars['fincommuneid'],0);
							}
							# Kommune abschließen:
							if ($this->piVars['finishcomm']) {
								tx_itaoshhmanager_database::finishCommune($this->piVars['fincommuneid'],1);
							}
							
							
							tx_itaoshhmanager_database::checkHeader();
							
							
						
						
							if ($this->piVars['offerid']) {
				...
				function getCommunePage() {
					...
					$markerArray['###TH_HEADER###'] = tx_itaoshhmanager_library::template_ausgeben($markerArray_th, 'TEMPLATE_TABLEHEADER', 'layout/be_template.html');		
					$markerArray['###IMAGEPATH###'] = $this->imagepath;	
					$markerArray['###INDEXPATH###'] = $this->indexpath;
					#$locat = $this->indexpath;
					$locat = urlencode('mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM1');
					...
					$html = utf8_encode(tx_itaoshhmanager_library::template_ausgeben($markerArray, 'TEMPLATE_COMMUNE_STARTPAGE', 'layout/be_template.html'));
					return $html;
				}
				...
				function getCommunesOverviewTable() {
					...
					while (list ($key, $val) = each ($allCommunes)) {
						...
						#$markerArray_td['###COMMUNE_NAME###'] = $val['titel'];
						if (tx_itaoshhmanager_general::hasRights(1,3)) {
							$markerArray_td['###COMMUNE_NAME###'] = '<a href="'.$this->indexpath.'&communeid='.$val['uid'].'" title="zu den Details der Kommune">';
							$markerArray_td['###COMMUNE_NAME###'] .= utf8_decode($val['titel']).'</a>';
						} else {
						...
						if (is_array($allSchools)) {
							while (list ($key2, $val2) = each ($allSchools)) {
								...
								if (tx_itaoshhmanager_general::hasRights(2,1)) {
									$markerArray_td['###REFED_SCHOOLS###'] .= utf8_decode(tx_itaoshhmanager_library::template_ausgeben($ma_schools, 'TEMPLATE_REFED_SCHOOLS', 'layout/be_template.html'));
								} else {
									$markerArray_td['###REFED_SCHOOLS###'] .= utf8_decode(tx_itaoshhmanager_library::template_ausgeben($ma_schools, 'TEMPLATE_REFED_SCHOOLS_NORIGHTS', 'layout/be_template.html'));
								}
							}
						} else {
							$markerArray_td['###REFED_SCHOOLS###'] = '';
						...
						#$markerArray_td['###EDIT_COMMUNENHEADER_LINK###'] = tx_itaoshhmanager_general::getEditLink($LANG->getLL('l_editCommunenheader'), 'tx_itaoshhmanager_communes', $val['ref_communepage'],array(), array(),1);	
						
						$markerArray_td['###EDIT_COMMUNENHEADER_LINK###'] ='<a href="#" onclick="window.location.href=\'alt_doc.php?returnUrl=\'+T3_THIS_LOCATION+\'%26backfromcommheader=1%26communeid='.$communeid.'&amp;edit[pages]['.($val['ref_communepage']+1).']=edit&columnsOnly=media\'; return false;" title="Kommunenheader bearbeiten">
						 	<img alt="Kommunenheader bearbeiten" title="Kommunenheader bearbeiten" src="../typo3conf/ext/itao_shh_manager/layout/images/editlogo.png">					 	
						 </a>';
							
						
						if ($val['is_finished']==1) {
							$markerArray_td['###FINISH_COMMUNE_LINK###'] = $this->indexpath.'&fincommuneid='.$val['uid'].'&reactcomm=1';
							$markerArray_td['###FINISHCOMM_LINKTEXT###'] = $LANG->getLL('l_reactCommune');
							$markerArray_td['###FINISHCOMM_IMAGE###'] = $this->imagepath2."outofArchive.png";
							$markerArray_td['###ACTIONCOMMUNE_JS###'] = $LANG->getLL('l_reactCommune_js');
							$markerArray_td['###COMMUNE_NAME###'].=' &nbsp;<img src="'.$this->imagepath2.'archiv-icon-16.png" title="Kommune ist abgeschlossen" width="16" height="16">';
							
						} else {
							$markerArray_td['###FINISH_COMMUNE_LINK###'] = $this->indexpath.'&fincommuneid='.$val['uid'].'&finishcomm=1';
							$markerArray_td['###FINISHCOMM_LINKTEXT###'] = $LANG->getLL('l_finishCommune');
							$markerArray_td['###FINISHCOMM_IMAGE###'] = $this->imagepath2."intoArchive2.png";
							$markerArray_td['###ACTIONCOMMUNE_JS###'] = $LANG->getLL('l_finishCommune_js');
						}
					
						$markerArray_td['###CLASS_DISPLAY###'] = (tx_itaoshhmanager_general::hasRights(1,3)) ? '' : 'display_none';
						$markerArray_td['###CLASS_DISPLAY_FINISHED###'] = (tx_itaoshhmanager_general::isInBeUserGroup($BE_USER->user['usergroup'], $this->tsvars['be_groupid_techadmin'])) ? '' : 'display_none';
											
						$html.= tx_itaoshhmanager_library::template_ausgeben($markerArray_td, 'TEMPLATE_TABLEROWS_COMMUNES', 'layout/be_template.html');
					}
					return $html;
				}
				...
				function getCommuneDetailsPage($communeid) {
					...
					tx_itaoshhmanager_database::checkFreigabe($communeid);
					
					
					tx_itaoshhmanager_database::checkHeader(1,$communeid);
					...
					$newCommPageUid = $communedata['ref_communepage'];
					$ma_det_sc['###NEW_SCHOOL_LINK###'] = tx_itaoshhmanager_general::getNewLink($LANG->getLL('l_newSchool'), 'tx_itaoshhmanager_schools', ($newCommPageUid+1),$tca_params, $back_params,1);
					...
					$ma_det_sc['###TH_HEADER_SCHOOLS###'] = tx_itaoshhmanager_library::template_ausgeben($markerArray_th, 'TEMPLATE_TABLEHEADER_SC', 'layout/be_template.html');	
					...
					if (tx_itaoshhmanager_general::hasRights(1,3)) {
						...
						$ma_det_sc['###EDIT_COMMUNENHEADER_LINK###'] ='<a href="#" onclick="window.location.href=\'alt_doc.php?returnUrl=\'+T3_THIS_LOCATION+\'%26backfromcommheader=1%26communeid='.$communeid.'&amp;edit[pages]['.($communedata['ref_communepage']+1).']=edit&columnsOnly=media\'; return false;" title="Kommunenheader bearbeiten">
						 	<img alt="Kommunenheader bearbeiten" title="Kommunenheader bearbeiten" src="../typo3conf/ext/itao_shh_manager/layout/images/editlogo.png"> <u>Kommunenheader bearbeiten</u>						 	
						 </a>';
					} else {
						$ma_det_sc['###EDIT_COMMUNE_LINK###'] = '';
						$ma_det_sc['###EDIT_COMMUNENHEADER_LINK###'] ='';
					}		
					$ma['###EDIT_COMMUNE_LINK###'] = $ma_det_sc['###EDIT_COMMUNE_LINK###'];	
					$ma['###EDIT_COMMUNENHEADER_LINK###'] = $ma_det_sc['###EDIT_COMMUNENHEADER_LINK###'];		
						
					$html = tx_itaoshhmanager_library::template_ausgeben($ma_det_sc, 'TEMPLATE_SCHOOL_OVERVIEWTABLE', 'layout/be_template.html');	
					
					if ($communedata['is_finished']==1) {
							$ma['###FINISH_COMMUNE_LINK###'] = $this->indexpath.'&communeid='.$communeid.'&fincommuneid='.$communedata['uid'].'&reactcomm=1';
							$ma['###FINISHCOMM_LINKTEXT###'] = $LANG->getLL('l_reactCommune');
							$ma['###FINISHCOMM_IMAGE###'] = $this->imagepath2."outofArchive.png";
							$ma['###ACTIONCOMMUNE_JS###'] = $LANG->getLL('l_reactCommune_js');
							$ma['###COMMUNE_TITLE###'].=' &nbsp;<img src="'.$this->imagepath2.'archiv-icon-16.png" title="Kommune ist abgeschlossen" width="16" height="16">';
							
						} else {
							$ma['###FINISH_COMMUNE_LINK###'] = $this->indexpath.'&communeid='.$communeid.'&fincommuneid='.$communedata['uid'].'&finishcomm=1';
							$ma['###FINISHCOMM_LINKTEXT###'] = $LANG->getLL('l_finishCommune');
							$ma['###FINISHCOMM_IMAGE###'] = $this->imagepath2."intoArchive2.png";
							$ma['###ACTIONCOMMUNE_JS###'] = $LANG->getLL('l_finishCommune_js');
						}
					
					$ma['###LINKID_KOMMUNENSEITE###'] = $communedata['ref_communepage']+1;
					
					$ma['###CLASS_DISPLAY_FINISHED###'] = (tx_itaoshhmanager_general::isInBeUserGroup($BE_USER->user['usergroup'], $this->tsvars['be_groupid_techadmin'])) ? '' : 'display_none';
					$html_oben =  tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPLATE_COMMUNE_DETAILPAGE', 'layout/be_template.html');
					/*
					$submoduls = array mit strings
					$active = integer des gerade aktiven tabs von 0 bis 2
					$content = der html-inhalt*/
					
					$submoduls[0] = $LANG->getLL('submodule_commune');#'Allgemeines zur Kommune';	
...
/*							if (tx_itaoshhmanager_general::hasRights(2,3) || tx_itaoshhmanager_general::hasRights(2,1)) {
								$markerArray_td['###SCHOOL_NAME###'] = '<a href="'.$this->indexpath.'&communeid='.$communeid.'&schoolid='.$val['uid'].'" title="zu den Details der Schule">';
							...
							*/
							...
							$markerArray_td['###EDIT_SCHOOL_LINK###'] = tx_itaoshhmanager_general::getEditLink($LANG->getLL('l_editSchool'), 'tx_itaoshhmanager_schools', $val['uid'],array(), $back_params,1,$LANG->getLL('l_editSchool'),1);
							#$markerArray_td['###EDIT_SCHOOL_LINK###'] .= tx_itaoshhmanager_general::getEditLink($LANG->getLL('l_editSchool'), 'pages', 522,array(), $back_params,1,$LANG->getLL('l_editSchool'),1);
							/*$markerArray_td['###EDIT_SCHOOL_LINK###'] .='<a href="#" onclick="window.location.href=\'alt_doc.php?returnUrl=\'+T3_THIS_LOCATION+\'%26communeid=4&amp;edit[pages][522]=edit&columnsOnly=field_headerimage\'; return false;" title="Schule bearbeiten">
 	<img alt="Schule bearbeiten" title="Schule bearbeiten" src="../typo3conf/ext/itao_shh_manager/layout/images/editlogo.png"> <u>Schul-Header bearbeiten</u>
 	<!--<span class="t3-icon t3-icon-actions t3-icon-actions-document t3-icon-document-open"></span>-->
 </a>';*/
 
 							/*$markerArray_td['###EDIT_SCHOOL_LINK###'] .='<a href="#" onclick="window.location.href=\'alt_doc.php?returnUrl=\'+T3_THIS_LOCATION+\'%26communeid=4&amp;edit[pages][522]=edit&columnsOnly=tx_templavoila_flex[data][sDEF][lDEF][field_headerimage]&disHelp=1\'; return false;" title="Schule bearbeiten">
 	<img alt="Schule bearbeiten" title="Schule bearbeiten" src="../typo3conf/ext/itao_shh_manager/layout/images/edit.png"> <u>Schul-Header bearbeiten</u>
 	<!--<span class="t3-icon t3-icon-actions t3-icon-actions-document t3-icon-document-open"></span>-->
 </a>';
 */
 							# alt
						 	/*$markerArray_td['###EDIT_SCHOOL_LINK###'] .='<a href="#" onclick="window.location.href=\'alt_doc.php?returnUrl=\'+T3_THIS_LOCATION+\'%26communeid='.$communeid.'&amp;edit[pages]['.$val['ref_schoolpage'].']=edit&columnsOnly=media\'; return false;" title="Schulheader bearbeiten">
						 	<img alt="Schulheader bearbeiten" title="Schulheader bearbeiten" src="../typo3conf/ext/itao_shh_manager/layout/images/editlogo.png"> <u>Schul-Header bearbeiten</u>
						 	<!--<span class="t3-icon t3-icon-actions t3-icon-actions-document t3-icon-document-open"></span>-->
						 </a>';
						 */
						 $markerArray_td['###EDIT_SCHOOL_LINK5###'] .='<a href="#" onclick="window.location.href=\'alt_doc.php?returnUrl=\'+T3_THIS_LOCATION+\'%26communeid='.$communeid.'&amp;edit[tx_itaoshhmanager_schools]['.$val['uid'].']=edit&columnsOnly=media\'; return false;" title="Schulheader bearbeiten">
						 	<img alt="Schulheader bearbeiten" title="Schulheader bearbeiten" src="../typo3conf/ext/itao_shh_manager/layout/images/editlogo.png"> <u>Schul-Header bearbeiten</u>
						 	<!--<span class="t3-icon t3-icon-actions t3-icon-actions-document t3-icon-document-open"></span>-->
						 </a>';
 
 
							#data[pages][522][tx_templavoila_flex][data][sDEF][lDEF][field_headerimage][vDEF]_list
							$markerArray_td['###LINKID_SCHULSEITE###'] = $schooldata['ref_schoolpage']+10;
							$markerArray_td['###COMMUNEID###'] = $communeid;
							$markerArray_td['###SCHOOLID###'] = $schoolid;
							
							
							$markerArray_td['###CLASS_DISPLAY###'] = (tx_itaoshhmanager_general::hasRights(2,3)) ? '' : 'display_none';
							$html.= tx_itaoshhmanager_library::template_ausgeben($markerArray_td, 'TEMPLATE_TABLEROWS_SCHOOLS', 'layout/be_template.html');
						}
					} else {
						$html = utf8_encode(tx_itaoshhmanager_library::template_ausgeben($markerArray_td, 'TEMPLATE_NO_SCHOOLS_TABLE', 'layout/be_template.html'));
					}
					return $html;
				}
				...
				function getSchoolDetailsPage($schoolid) {
					global $LANG, $BE_USER;
					$GLOBALS["BE_USER"]->setAndSaveSessionData ('search_value', "");
					tx_itaoshhmanager_database::checkHeader(2,$schoolid);
					...
					$ma_det_sc['###SCHOOL_SHORT###'] = $schooldata['s_short'];
					...
					$ma_det_sc['###SC_STATUS###'] = $allPhasenSchool[$schooldata['ref_status']]['title'];
					$ma_det_sc['###STATUS_LABEL###'] = $allPhasenSchool[$schooldata['ref_status']]['title'];
					$ma_det_sc['###STATUS_IMAGE###'] = $this->imagepath2.$allPhasenSchool[$schooldata['ref_status']]['st_image'];
					...
					$ma_det_sc['###EDITLINK###'] = $ma['###EDITLINK###'] = tx_itaoshhmanager_general::getEditLink("".$LANG->getLL('l_editSchool'), 'tx_itaoshhmanager_schools', $schoolid,array(), $back_params,1,$LANG->getLL('l_editSchool'));
					
						 	$ma_det_sc['###EDITLINK5###'] .='<a href="#" onclick="window.location.href=\'alt_doc.php?returnUrl=\'+T3_THIS_LOCATION+\'%26communeid='.$communeid.'&amp;edit[pages]['.$schooldata['ref_schoolpage'].']=edit&columnsOnly=media\'; return false;" title="Schulheader bearbeiten">
						 	<img alt="Schulheader bearbeiten" title="Schulheader bearbeiten" src="../typo3conf/ext/itao_shh_manager/layout/images/editlogo.png"> <u>Schul-Header bearbeiten</u>
						 	<!--<span class="t3-icon t3-icon-actions t3-icon-actions-document t3-icon-document-open"></span>-->
						 </a>';
					...
					# Anzahl FE-User:
					$ma_det_sc['###ANZ_FE_USER###'] = tx_itaoshhmanager_database::getAnzAccountBySchool($schoolid,1,0);
					# Anzahl Vorschlaege:	 
					$allOffers = tx_itaoshhmanager_database::getAllOffersBySchool($schoolid,0,1, '',1);
					$ma_det_sc['###ANZ_OFFERS###'] = count($allOffers);
					
					$ma['###LINKID_SCHULSEITE###'] = $ma_det_sc['###LINKID_SCHULSEITE###'] = $schooldata['ref_schoolpage']+10;
					$ma['###COMMUNEID###'] = $ma_det_sc['###COMMUNEID###'] = $communeid;
					$ma['###SCHOOLID###'] = $ma_det_sc['###SCHOOLID###'] = $schoolid;
					$html = tx_itaoshhmanager_library::template_ausgeben($ma_det_sc, 'TEMPLATE_SCHOOL_DETAIL_CONTENT', 'layout/be_template.html');	
					...
					$html_oben =  tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPLATE_SCHOOL_DETAILPAGE', 'layout/be_template.html');
					/*
					$submoduls = array mit strings
					$active = integer des gerade aktiven tabs von 0 bis 2
					$content = der html-inhalt*/
...
*/					
				...
				function getBoxBaseVorbereitung($schoolid) {
					...
					$html = tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_BASIC', 'layout/be_template.html');
					
					return $html;
				}
				
				function getBoxBodyVorbereitung($schoolid) {
					$html = tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_BODY_VORBEREITUNG', 'layout/be_template.html');
					#$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_JS', 'layout/be_template.html');
					return $html;
				}
				...
				function getBoxBodyOnline($schoolid) {
					$html = tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_BODY_ONLINE', 'layout/be_template.html');
					#$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_JS', 'layout/be_template.html');
					return $html;
				}
				...
				function getBoxBodyErgebnis($schoolid) {
					$html = tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_BODY_ERGEBNIS', 'layout/be_template.html');
					$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_JS', 'layout/be_template.html');
					return $html;
				}
				...
				function getSidebox($schoolid) {
					...
					$html .= tx_itaoshhmanager_library::template_ausgeben($ma_det_sc, 'SIDEBOX_SCHOOL', 'layout/be_template.html');
					return $html;
				}
				...
				function getBenutzerPage($schoolid) {
					...
					$locat = urlencode('mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM1');
					$ma['###JS_EDIT###'] = tx_itaoshhmanager_library::js_fuer_ds_bearbeiten($locat);
					$html_oben =  tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPLATE_USER_DETAILPAGE', 'layout/be_template.html');
					...
					$html = tx_itaoshhmanager_library::template_ausgeben($ma_det_sc, 'TEMPLATE_USER_DETAIL_CONTENT', 'layout/be_template.html');	
...
#!#!#						$submoduls[8] = $LANG->getLL('submodule_school_editfeuser');
				...
				function getBoxBodyVerwKommune($schoolid,$groupid,$communeid2 = 0) {
					...
					$ma['###GROUPID###'] = $groupid;
					$markerArray['###GENERATE_ACCOUNTS###'] = tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPL_GENERATE_ACCOUNTS', 'layout/be_template.html');									
					...
					if (is_file($_SERVER['DOCUMENT_ROOT']."/".$this->tsvars['accountpath'].$pdfdateiname)) {
						...
						$markerArray['###DOWNLOAD_ACCOUNTS###'] = tx_itaoshhmanager_library::template_ausgeben($ma_dl,'TEMP_DOWNLOAD_ACCOUNTS','layout/be_template.html');
						
						$ma_upd['###PFAD_UPDATE_FEACCOUNTS###'] = $this->indexpath.'&communeid='.$communeid.'&schoolid='.$schoolid.'&benutzer=1&anz_acc_upd_'.$groupid.'=1';
						$markerArray['###UPDATE_ACCOUNTS###'] = tx_itaoshhmanager_library::template_ausgeben($ma_upd,'TEMP_UPDATE_ACCOUNTS','layout/be_template.html');
					} else {						
						$markerArray['###DOWNLOAD_ACCOUNTS###'] = "";#" [noch kein PDF vorhanden]";
						$markerArray['###UPDATE_ACCOUNTS###'] = "";
					}
					
					$html = tx_itaoshhmanager_library::template_ausgeben($markerArray,$templPart , 'layout/be_template.html');
					#$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_JS', 'layout/be_template.html');
					return $html;
				}
				...
				function getSideboxUser($schoolid) {
					...
					$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'SIDEBOX_USER', 'layout/be_template.html');
					return $html;
				}
...
//							$sortArr[6]['title'] = 'help';				
					...
					# Content über Tabs:
					$ma['###SCHOOL_TITLE###'] = $schooldata['title'];
					$ma['###SC_ORT###'] = ($schooldata['city']!="") ? '('.$schooldata['city'].')' : '';$locat = urlencode('mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM1');
					$ma['###JS_EDIT###'] = tx_itaoshhmanager_library::js_fuer_ds_bearbeiten($locat);
					$html_oben =  tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPLATE_OFFERS_DETAILPAGE', 'layout/be_template.html');
					...
					$ma_det_sc['###ANZ_ERGEBNISSE###'] = $this->anz_ergebnisse;
						...
						$ma_det_sc['###LINK_DOC_GEN###'] = tx_itaoshhmanager_library::template_ausgeben($ma_doc, 'T_GENERATE_DOC', 'layout/be_template.html');	
					...
					$html = tx_itaoshhmanager_library::template_ausgeben($ma_det_sc, 'TEMPLATE_OFFERS_DETAIL_CONTENT', 'layout/be_template.html');	
...
#!#!#						$submoduls[8] = $LANG->getLL('submodule_school_editfeuser');
				...
				function getSideboxOffers($schoolid) {
					...
					$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'SIDEBOX_OFFERS', 'layout/be_template.html');
					return $html;
				}
				...
				function getOffersTableRows($schoolid,$sort_nach = 1, $sort_dir="asc") {
					...
					$allOffers = tx_itaoshhmanager_database::getAllOffersBySchool($schoolid,0,$sort_nach, $sort_dir);
...
//							$markerArray['###HELP_WANTED###'] 	=  ($val['costs_help']) ? 'ja' : 'nein';
							...
							$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'TEMPLATE_TABLEROWS_OFFERS', 'layout/be_template.html');
						}
					} else {
						$html .=tx_itaoshhmanager_library::template_ausgeben($markerArray, 'TEMPLATE_NO_OFFERS', 'layout/be_template.html');
					}
				...
				function getOfferDetailPage($schoolid, $offerid) {
					...
					$ma['###SC_ORT###'] = ($schooldata['city']!="") ? '('.$schooldata['city'].')' : '';$locat = urlencode('mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM1');
					$ma['###JS_EDIT###'] = tx_itaoshhmanager_library::js_fuer_ds_bearbeiten($locat);
					$html_oben =  tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPLATE_OFFER_VIEW', 'layout/be_template.html');
					...
					#$ma_off_det['###FUNKTIONSLEISTE###'] = ($offerData['status'] > 1) ? '' : tx_itaoshhmanager_library::template_ausgeben($off_funcs, 'OFFER_FUNCTIONS', 'layout/be_template.html');
					...
					$ma_off_det['###FUNKTIONSLEISTE###'] = tx_itaoshhmanager_library::template_ausgeben($off_funcs, 'OFFER_FUNCTIONS', 'layout/be_template.html');
...
//					$ma_off_det['###OFF_HELP###'] = ($offerData['costs_help']) ? '' : 'keine ';				
					...
					if (is_array($comments)) { #$offerData['idea_from'] && 
						...
						$ma_off_det['###OFF_COMMENTS###'] = '<span class="listgrey">[nicht kommentiert]</span>';
					}
					...
					$html = tx_itaoshhmanager_library::template_ausgeben($ma_off_det, 'TEMPLATE_OFFER_VIEW_CONTENT', 'layout/be_template.html');	
...
#!#!#						$submoduls[8] = $LANG->getLL('submodule_school_editfeuser');
				...
				function getSideboxOffersDetail($schoolid, $offerid) {
					...
					$html .= tx_itaoshhmanager_library::template_ausgeben($ma_off_det, 'SIDEBOX_OFFER_DETAIL', 'layout/be_template.html');
					return $html;
				}
				...
				function getBeUserPage($communeid, $schoolid) {					
					...
					$locat = urlencode('mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM1');
					$ma['###JS_EDIT###'] = tx_itaoshhmanager_library::js_fuer_ds_bearbeiten($locat);
					$html_oben =  tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPLATE_BEUSER_DETAILPAGE', 'layout/be_template.html');
					...
					$html = tx_itaoshhmanager_library::template_ausgeben($ma_det_sc, 'TEMPLATE_BEUSER_DETAIL_CONTENT', 'layout/be_template.html');	
...
#!#!#						$submoduls[8] = $LANG->getLL('submodule_school_editfeuser');
				...
				function getBoxBodyBeUser($communeid,$schoolid,$groupid) {
					...
					$ma['###GROUPID###'] = $groupid;
					$markerArray['###GENERATE_ACCOUNTS###'] = tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPL_GENERATE_BEACCOUNTS', 'layout/be_template.html');									
					...
					if (is_file($_SERVER['DOCUMENT_ROOT']."/".$this->tsvars['beaccountpath'].$pdfdateiname)) {
						...
						$markerArray['###DOWNLOAD_ACCOUNTS###'] = tx_itaoshhmanager_library::template_ausgeben($ma_dl,'TEMP_DOWNLOAD_BEACCOUNTS','layout/be_template.html');
						
						$ma_upd['###PFAD_UPDATE_BEACCOUNTS###'] = $this->indexpath.'&communeid='.$communeid.'&schoolid='.$schoolid.'&beuser=1&anz_beacc_upd_'.$groupid.'=1';
						$markerArray['###UPDATE_ACCOUNTS###'] = tx_itaoshhmanager_library::template_ausgeben($ma_upd,'TEMP_UPDATE_BEACCOUNTS','layout/be_template.html');
					} else {						
						$markerArray['###DOWNLOAD_ACCOUNTS###'] = "";#" [noch kein PDF vorhanden]";
						$markerArray['###UPDATE_ACCOUNTS###'] = "";
					}
				...
				function getBenutzerEditPage($communeid,$schoolid) {
					...
					$ma['###SC_ORT###'] = ($schooldata['city']!="") ? '('.$schooldata['city'].')' : '';$locat = urlencode('mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM1');
					$ma['###JS_EDIT###'] = tx_itaoshhmanager_library::js_fuer_ds_bearbeiten($locat);
					$html_oben =  tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPLATE_FEUSER_EDITPAGE', 'layout/be_template.html');
				...
				#			$search_value_group = $GLOBALS["BE_USER"]->getSessionData("filter_usergroups");
					...
					$html = tx_itaoshhmanager_library::template_ausgeben($ma_det_sc, 'TEMPLATE_FEUSER_EDIT_CONTENT', 'layout/be_template.html');	
					...
					if (is_array($allOffers)) {
						...
						while (list ($key, $val) = each ($allOffers)) {
							...
							*/
							...
							$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'TEMPLATE_TABLEROWS_USERSEDIT', 'layout/be_template.html');
						}
					} else {
						$html .=tx_itaoshhmanager_library::template_ausgeben($markerArray, 'TEMPLATE_NO_USERSEDIT', 'layout/be_template.html');
					}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/itao_shh_manager/mod1/index.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/itao_shh_manager/mod1/index.php']);
}
...
// Make instance:
$SOBE = t3lib_div::makeInstance('tx_itaoshhmanager_module1');
$SOBE->init();
