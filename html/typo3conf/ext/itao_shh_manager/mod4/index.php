...
$LANG->includeLLFile('EXT:itao_shh_manager/mod4/locallang.xml');
 ...
 */
class  tx_itaoshhmanager_module4 extends t3lib_SCbase {
				var $pageinfo;
				...
				function main()	{
					...
					$this->doc->setModuleTemplate(t3lib_extMgm::extPath('itao_shh_manager') . 'mod4/mod_template.html');
          ...
          $has_pruefer_rights = 1;
				...
				function getBackLink($func_nr) {
					...
					if ($this->piVars['communeid']) {
						$backlink = '
							<a href="/typo3/mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM4">
...
/*'*/
				...
				function moduleContent()	{
					...
					*/
					...
					$this->indexpath = "/typo3/mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM4";		
					...
					switch((string)$this->MOD_SETTINGS['function'])	{
						case 1:
							...
							} elseif ($this->piVars['benutzer_school']) {
								$content=$this->getBenutzerPage_school($this->piVars['schoolid']);
				...
				function getCommunePage() {
					...
					$markerArray['###TH_HEADER###'] = tx_itaoshhmanager_library::template_ausgeben($markerArray_th, 'TEMPLATE_TABLEHEADER', 'layout/be_template_mod4.html');		
					$markerArray['###IMAGEPATH###'] = $this->imagepath;	
					$markerArray['###INDEXPATH###'] = $this->indexpath;
					#$locat = $this->indexpath;
					$locat = urlencode('mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM4');
					...
					$html = utf8_encode(tx_itaoshhmanager_library::template_ausgeben($markerArray, 'TEMPLATE_COMMUNE_STARTPAGE', 'layout/be_template_mod4.html'));
					return $html;
				}
				...
				function getCommunesOverviewTable() {
					...
					while (list ($key, $val) = each ($allCommunes)) {
						...
						if (tx_itaoshhmanager_general::hasRights(1,3)) {
							$markerArray_td['###COMMUNE_NAME###'] = '<a href="'.$this->indexpath.'&communeid='.$val['uid'].'&benutzer=1" title="zu den Details der Kommune">';
						...
						if (is_array($allSchools)) {
							while (list ($key2, $val2) = each ($allSchools)) {
								...
								if (tx_itaoshhmanager_general::hasRights(2,1)) {
									$markerArray_td['###REFED_SCHOOLS###'] .= utf8_decode(tx_itaoshhmanager_library::template_ausgeben($ma_schools, 'TEMPLATE_REFED_SCHOOLS', 'layout/be_template_mod4.html'));
								} else {
									$markerArray_td['###REFED_SCHOOLS###'] .= utf8_decode(tx_itaoshhmanager_library::template_ausgeben($ma_schools, 'TEMPLATE_REFED_SCHOOLS_NORIGHTS', 'layout/be_template_mod4.html'));
								}
							}
						} else {
							$markerArray_td['###REFED_SCHOOLS###'] = '';
						...
						$html.= tx_itaoshhmanager_library::template_ausgeben($markerArray_td, 'TEMPLATE_TABLEROWS_COMMUNES', 'layout/be_template_mod4.html');
					}
					return $html;
				}
				...
				function getCommuneDetailsPage($communeid) {
					...
					$ma_det_sc['###TH_HEADER_SCHOOLS###'] = tx_itaoshhmanager_library::template_ausgeben($markerArray_th, 'TEMPLATE_TABLEHEADER_SC', 'layout/be_template_mod4.html');	
					...
					$html = tx_itaoshhmanager_library::template_ausgeben($ma_det_sc, 'TEMPLATE_SCHOOL_OVERVIEWTABLE', 'layout/be_template_mod4.html');	
					
					
					$html_oben =  tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPLATE_COMMUNE_DETAILPAGE', 'layout/be_template_mod4.html');
					/*
...
#								if (tx_itaoshhmanager_general::isInBeUserGroup($BE_USER->user['usergroup'], $this->tsvars['be_groupid_redadmin_kommune']) &&  !$BE_USER->user['admin']) {
									$linkerg = '&benutzer_school=1';
#								}
								$markerArray_td['###SCHOOL_NAME###'] = '<a href="'.$this->indexpath.'&communeid='.$communeid.'&schoolid='.$val['uid'].$linkerg.'" title="zu den Details der Schule">';
							...
							$markerArray_td['###CLASS_DISPLAY###'] = (tx_itaoshhmanager_general::hasRights(2,3)) ? '' : 'display_none';
							$html.= tx_itaoshhmanager_library::template_ausgeben($markerArray_td, 'TEMPLATE_TABLEROWS_SCHOOLS', 'layout/be_template_mod4.html');
						}
					} else {
						$html = utf8_encode(tx_itaoshhmanager_library::template_ausgeben($markerArray_td, 'TEMPLATE_NO_SCHOOLS_TABLE', 'layout/be_template_mod4.html'));
					}
					return $html;
				}
				...
				function getSchoolDetailsPage($schoolid) {
					...
					$html = tx_itaoshhmanager_library::template_ausgeben($ma_det_sc, 'TEMPLATE_SCHOOL_DETAIL_CONTENT', 'layout/be_template_mod4.html');	
					...
					$html_oben =  tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPLATE_SCHOOL_DETAILPAGE', 'layout/be_template_mod4.html');
					/*
					$submoduls = array mit strings
					$active = integer des gerade aktiven tabs von 0 bis 2
					$content = der html-inhalt*/
...
*/					
				...
				function getBoxBaseVorbereitung($schoolid) {
					...
					$markerArray['###EDITHEADER###'] = ($schulstatus > 1) ? tx_itaoshhmanager_library::template_ausgeben($markerArray, 'SWITCHBACK', 'layout/be_template_mod4.html'): '';	#Switch back	
					...
					$html = tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_BASIC', 'layout/be_template_mod4.html');
				...
				function getBoxBodyVorbereitung($schoolid,$schooldata,$schulstatus) {
					...
					$html = tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_BODY_VORBEREITUNG', 'layout/be_template_mod4.html');
					#$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_JS', 'layout/be_template_mod4.html');
					return $html;
				}
				...
				function getBoxBaseOnline($schoolid) {
					...
					$markerArray['###EDITHEADER###'] = ($schulstatus > 2) ? tx_itaoshhmanager_library::template_ausgeben($markerArray, 'SWITCHBACK', 'layout/be_template_mod4.html'): '';	#Switch back		
				...
				function getBoxBodyOnline($schoolid,$schooldata,$schulstatus) {
					...
					$markerArray['###SCHALTE_AKTIV###'] = ($schulstatus == $akt_status && $conditionsOkay) ? '' : ' disabled="disabled" ';					
					$markerArray['###FORMURL###'] = $this->indexpath.'&smnr=1&communeid='.$schooldata['ref_commune'].'&schoolid='.$schoolid.'';
					$html = tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_BODY_ONLINE', 'layout/be_template_mod4.html');
					#$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_JS', 'layout/be_template_mod4.html');
					return $html;
				}
				...
				function getBoxBaseAbstimmung($schoolid) {
					...
					$markerArray['###EDITHEADER###'] = ($schulstatus > 3) ? tx_itaoshhmanager_library::template_ausgeben($markerArray, 'SWITCHBACK', 'layout/be_template_mod4.html'): '';	#Switch back		
				...
				function getBoxBodyAbstimmung($schoolid,$schooldata,$schulstatus) {
					...
					$html = tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_BODY_ABSTIMMUNG', 'layout/be_template_mod4.html');
					$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_JS', 'layout/be_template_mod4.html');
					return $html;
				}
				...
				function getBoxBodyErgebnis($schoolid,$schooldata,$schulstatus) {
					...
					$html = tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_BODY_ERGEBNIS', 'layout/be_template_mod4.html');
					$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_JS', 'layout/be_template_mod4.html');
					return $html;
				}
				...
				function getSidebox($schoolid) {
					...
					$html .= tx_itaoshhmanager_library::template_ausgeben($ma_det_sc, 'SIDEBOX_SCHOOL', 'layout/be_template_mod4.html');
					return $html;
				}
				...
				function getBenutzerPage($schoolid) {
					...
					$locat = urlencode('mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM4');
					$ma['###JS_EDIT###'] = tx_itaoshhmanager_library::js_fuer_ds_bearbeiten($locat);
					$html_oben =  tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPLATE_USER_DETAILPAGE', 'layout/be_template_mod4.html');
					...
					$html = tx_itaoshhmanager_library::template_ausgeben($ma_det_sc, 'TEMPLATE_USER_DETAIL_CONTENT', 'layout/be_template_mod4.html');	
...
#					t3lib_utility_debug::debug("active=2");
					$active = 2;
					$html = $html_oben .$breadcrumb. tx_itaoshhmanager_navigation::getSubmodulNavi($submoduls,$active, $html, 'long');
					$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_JS', 'layout/be_template_mod4.html');
					return $html;
				  ...
				  * Hier wird eine Oberfläche generiert, auf der man für die ausgewählte Schule FE-Benutzer anlegen kann
				  */
				function getBenutzerPage_school($schoolid) {
					global $LANG, $BE_USER;
					#t3lib_utility_debug::debug("in _school");
					$GLOBALS["BE_USER"]->setAndSaveSessionData ('search_value', "");
					$schooldata = tx_itaoshhmanager_database::getSchoolById($schoolid);
					$communeid = $this->piVars['communeid']; #$schooldata['ref_commune'];
					$communedata = tx_itaoshhmanager_database::getCommunesById($communeid);
...
#						$submoduls[1] = "Aktueller Status/Phase";#'Phasen der Schule';#$LANG->getLL('submodule_school');
					...
					if (tx_itaoshhmanager_general::hasRights(1,1)) { # wenn jemand die Komune ansehen darf
						...
						if ($schoolid !=0 ) {
							#$submoduls[2] = 'Zugangsdaten anlegen';
							$submoduls[22] = 'Mitarbeiter Schule anlegen';#'Zugangsdaten anlegen';
						}
...
#							$submoduls[5] = 'Administrator Schule anlegen';
					...
					#t3lib_utility_debug::debug("active=");
					$active = 22;
					$html = $html_oben .$breadcrumb. tx_itaoshhmanager_navigation::getSubmodulNavi($submoduls,$active, $html, 'long');
					$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_JS', 'layout/be_template_mod4.html');
					return $html;
				...
				function getBoxBodyVerwKommune($schoolid,$groupid,$communeid2 = 0) {
					...
					$ma['###GROUPID###'] = $groupid;
					$markerArray['###GENERATE_ACCOUNTS###'] = tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPL_GENERATE_ACCOUNTS', 'layout/be_template_mod4.html');	
					if ($this->piVars['benutzer_school']) {
						$param_ende = '&benutzer_school=1';
					} else {
						$param_ende = '&benutzer=1';
					}										
					$markerArray['###FORM_ACTION_GENACC###'] = $this->indexpath.'&communeid='.$communeid.'&schoolid='.$schoolid.$param_ende ;#'&benutzer=1';
					...
					$pdfdateiname_link = $pdfdateiname."?".time();
					if (is_file($_SERVER['DOCUMENT_ROOT']."/".$this->tsvars['accountpath'].$pdfdateiname)) {
						$ma_dl['###PFAD_DOWNLOAD###'] = 'http://'.$_SERVER['HTTP_HOST']."/".$this->tsvars['accountpath'].$pdfdateiname_link;
						$markerArray['###DOWNLOAD_ACCOUNTS###'] = tx_itaoshhmanager_library::template_ausgeben($ma_dl,'TEMP_DOWNLOAD_ACCOUNTS','layout/be_template_mod4.html');
						
						$ma_upd['###PFAD_UPDATE_FEACCOUNTS###'] = $this->indexpath.'&communeid='.$communeid.'&schoolid='.$schoolid.'&benutzer=1&anz_acc_upd_'.$groupid.'=1';
						$markerArray['###UPDATE_ACCOUNTS###'] = tx_itaoshhmanager_library::template_ausgeben($ma_upd,'TEMP_UPDATE_ACCOUNTS','layout/be_template_mod4.html');
					} else {						
						$markerArray['###DOWNLOAD_ACCOUNTS###'] = "";#" [noch kein PDF vorhanden]";
						$markerArray['###UPDATE_ACCOUNTS###'] = "";
					}
					
					$html = tx_itaoshhmanager_library::template_ausgeben($markerArray,$templPart , 'layout/be_template_mod4.html');
					#$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_JS', 'layout/be_template_mod4.html');
					return $html;
				}
				...
				function getSideboxUser($schoolid) {
					...
					$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'SIDEBOX_USER', 'layout/be_template_mod4.html');
					return $html;
				}
...
//							$sortArr[6]['title'] = 'help';				
					...
					# Content über Tabs:
					$ma['###SCHOOL_TITLE###'] = $schooldata['title'];
					$ma['###SC_ORT###'] = ($schooldata['city']!="") ? '('.$schooldata['city'].')' : '';$locat = urlencode('mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM4');
					$ma['###JS_EDIT###'] = tx_itaoshhmanager_library::js_fuer_ds_bearbeiten($locat);
					$html_oben =  tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPLATE_OFFERS_DETAILPAGE', 'layout/be_template_mod4.html');
					...
					$ma_det_sc['###ANZ_ERGEBNISSE###'] = $this->anz_ergebnisse;
						...
						$ma_det_sc['###LINK_DOC_GEN###'] = tx_itaoshhmanager_library::template_ausgeben($ma_doc, 'T_GENERATE_DOC', 'layout/be_template_mod4.html');	
					...
					$html = tx_itaoshhmanager_library::template_ausgeben($ma_det_sc, 'TEMPLATE_OFFERS_DETAIL_CONTENT', 'layout/be_template_mod4.html');	
...
#!#!#						$submoduls[8] = $LANG->getLL('submodule_school_editfeuser');
				...
				function getSideboxOffers($schoolid) {
					...
					$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'SIDEBOX_OFFERS', 'layout/be_template_mod4.html');
					return $html;
				}
...
//							$markerArray['###HELP_WANTED###'] 	=  ($val['costs_help']) ? 'ja' : 'nein';
							...
							$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'TEMPLATE_TABLEROWS_OFFERS', 'layout/be_template_mod4.html');
						}
					} else {
						$html .=tx_itaoshhmanager_library::template_ausgeben($markerArray, 'TEMPLATE_NO_OFFERS', 'layout/be_template_mod4.html');
					}
				...
				function getOfferDetailPage($schoolid, $offerid) {
					...
					$ma['###SC_ORT###'] = ($schooldata['city']!="") ? '('.$schooldata['city'].')' : '';$locat = urlencode('mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM4');
					$ma['###JS_EDIT###'] = tx_itaoshhmanager_library::js_fuer_ds_bearbeiten($locat);
					$html_oben =  tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPLATE_OFFER_VIEW', 'layout/be_template_mod4.html');
					...
					#$ma_off_det['###FUNKTIONSLEISTE###'] = ($offerData['status'] > 1) ? '' : tx_itaoshhmanager_library::template_ausgeben($off_funcs, 'OFFER_FUNCTIONS', 'layout/be_template_mod4.html');
					...
					$ma_off_det['###FUNKTIONSLEISTE###'] = tx_itaoshhmanager_library::template_ausgeben($off_funcs, 'OFFER_FUNCTIONS', 'layout/be_template_mod4.html');
...
//					$ma_off_det['###OFF_HELP###'] = ($offerData['costs_help']) ? '' : 'keine ';				
					...
					$html = tx_itaoshhmanager_library::template_ausgeben($ma_off_det, 'TEMPLATE_OFFER_VIEW_CONTENT', 'layout/be_template_mod4.html');	
...
#!#!#						$submoduls[8] = $LANG->getLL('submodule_school_editfeuser');
				...
				function getSideboxOffersDetail($schoolid, $offerid) {
					...
					$html .= tx_itaoshhmanager_library::template_ausgeben($ma_off_det, 'SIDEBOX_OFFER_DETAIL', 'layout/be_template_mod4.html');
					return $html;
				}
				...
				function getBeUserPage($communeid, $schoolid) {					
					...
					$locat = urlencode('mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM4');
					$ma['###JS_EDIT###'] = tx_itaoshhmanager_library::js_fuer_ds_bearbeiten($locat);
					$html_oben =  tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPLATE_BEUSER_DETAILPAGE', 'layout/be_template_mod4.html');
					...
					$html = tx_itaoshhmanager_library::template_ausgeben($ma_det_sc, 'TEMPLATE_BEUSER_DETAIL_CONTENT', 'layout/be_template_mod4.html');	
...
*/					
				...
				function getBoxBodyBeUser($communeid,$schoolid,$groupid) {
					...
					$ma['###GROUPID###'] = $groupid;
					$markerArray['###GENERATE_ACCOUNTS###'] = tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPL_GENERATE_BEACCOUNTS', 'layout/be_template_mod4.html');									
					...
					if (is_file($_SERVER['DOCUMENT_ROOT']."/".$this->tsvars['beaccountpath'].$pdfdateiname)) {
						...
						$markerArray['###DOWNLOAD_ACCOUNTS###'] = tx_itaoshhmanager_library::template_ausgeben($ma_dl,'TEMP_DOWNLOAD_BEACCOUNTS','layout/be_template_mod4.html');
						
						$ma_upd['###PFAD_UPDATE_BEACCOUNTS###'] = $this->indexpath.'&communeid='.$communeid.'&schoolid='.$schoolid.'&beuser=1&anz_beacc_upd_'.$groupid.'=1';
						$markerArray['###UPDATE_ACCOUNTS###'] = tx_itaoshhmanager_library::template_ausgeben($ma_upd,'TEMP_UPDATE_BEACCOUNTS','layout/be_template_mod4.html');
					} else {						
						$markerArray['###DOWNLOAD_ACCOUNTS###'] = "";#" [noch kein PDF vorhanden]";
						$markerArray['###UPDATE_ACCOUNTS###'] = "";
					}
				...
				function getBenutzerEditPage($communeid,$schoolid) {
					...
					$ma['###SC_ORT###'] = ($schooldata['city']!="") ? '('.$schooldata['city'].')' : '';$locat = urlencode('mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM4');
					$ma['###JS_EDIT###'] = tx_itaoshhmanager_library::js_fuer_ds_bearbeiten($locat);
					$html_oben =  tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPLATE_FEUSER_EDITPAGE', 'layout/be_template_mod4.html');
				...
				#			$search_value_group = $GLOBALS["BE_USER"]->getSessionData("filter_usergroups");
					...
					$html = tx_itaoshhmanager_library::template_ausgeben($ma_det_sc, 'TEMPLATE_FEUSER_EDIT_CONTENT', 'layout/be_template_mod4.html');	
					...
					if (is_array($allOffers)) {
						...
						while (list ($key, $val) = each ($allOffers)) {
							...
							*/
							...
							$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'TEMPLATE_TABLEROWS_USERSEDIT', 'layout/be_template_mod4.html');
						}
					} else {
						$html .=tx_itaoshhmanager_library::template_ausgeben($markerArray, 'TEMPLATE_NO_USERSEDIT', 'layout/be_template_mod4.html');
					}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/itao_shh_manager/mod4/index.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/itao_shh_manager/mod4/index.php']);
}
...
// Make instance:
$SOBE = t3lib_div::makeInstance('tx_itaoshhmanager_module4');
$SOBE->init();
