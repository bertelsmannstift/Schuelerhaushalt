...
$LANG->includeLLFile('EXT:itao_shh_manager/mod2/locallang.xml');
 ...
 */
class  tx_itaoshhmanager_module2 extends t3lib_SCbase {
				var $pageinfo;
				...
				function main()	{
					...
					$this->doc->setModuleTemplate(t3lib_extMgm::extPath('itao_shh_manager') . 'mod2/mod_template.html');
          ...
          $has_pruefer_rights = 1;
				...
				function getBackLink($func_nr) {
					...
					if ($this->piVars['communeid']) {
						$backlink = '
							<a href="/typo3/mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM2">
...
/*'*/
				...
				function moduleContent()	{
					...
					*/
					...
					$this->indexpath = "/typo3/mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM2";		
					...
					switch((string)$this->MOD_SETTINGS['function'])	{
						case 1:
							...
							if ($this->piVars['offerid']) {
								...
								$content=$this->getBeUserPage($this->piVars['communeid']);
							} elseif ($this->piVars['beuser_school']) {
								$content=$this->getBeUserPage_school($this->piVars['communeid'],$this->piVars['schoolid']);
				...
				function getCommunePage() {
					...
					$markerArray['###TH_HEADER###'] = tx_itaoshhmanager_library::template_ausgeben($markerArray_th, 'TEMPLATE_TABLEHEADER', 'layout/be_template_mod2.html');		
					$markerArray['###IMAGEPATH###'] = $this->imagepath;	
					$markerArray['###INDEXPATH###'] = $this->indexpath;
					#$locat = $this->indexpath;
					$locat = urlencode('mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM2');
					...
					$html = utf8_encode(tx_itaoshhmanager_library::template_ausgeben($markerArray, 'TEMPLATE_COMMUNE_STARTPAGE', 'layout/be_template_mod2.html'));
					return $html;
				}
				...
				function getCommunesOverviewTable() {
					...
					while (list ($key, $val) = each ($allCommunes)) {
						...
						if (tx_itaoshhmanager_general::hasRights(1,3)) {
							$markerArray_td['###COMMUNE_NAME###'] = '<a href="'.$this->indexpath.'&communeid='.$val['uid'].'&beuser=1" title="zu den Details der Kommune">';
						...
						if (is_array($allSchools)) {
							while (list ($key2, $val2) = each ($allSchools)) {
								...
								if (tx_itaoshhmanager_general::hasRights(2,1)) {
									$markerArray_td['###REFED_SCHOOLS###'] .= utf8_decode(tx_itaoshhmanager_library::template_ausgeben($ma_schools, 'TEMPLATE_REFED_SCHOOLS', 'layout/be_template_mod2.html'));
								} else {
									$markerArray_td['###REFED_SCHOOLS###'] .= utf8_decode(tx_itaoshhmanager_library::template_ausgeben($ma_schools, 'TEMPLATE_REFED_SCHOOLS_NORIGHTS', 'layout/be_template_mod2.html'));
								}
							}
						} else {
							$markerArray_td['###REFED_SCHOOLS###'] = '';
						...
						$html.= tx_itaoshhmanager_library::template_ausgeben($markerArray_td, 'TEMPLATE_TABLEROWS_COMMUNES', 'layout/be_template_mod2.html');
					}
					return $html;
				}
				...
				function getCommuneDetailsPage($communeid) {
					...
					$ma_det_sc['###TH_HEADER_SCHOOLS###'] = tx_itaoshhmanager_library::template_ausgeben($markerArray_th, 'TEMPLATE_TABLEHEADER_SC', 'layout/be_template_mod2.html');	
					...
					$html = tx_itaoshhmanager_library::template_ausgeben($ma_det_sc, 'TEMPLATE_SCHOOL_OVERVIEWTABLE', 'layout/be_template_mod2.html');	
					
					
					$html_oben =  tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPLATE_COMMUNE_DETAILPAGE', 'layout/be_template_mod2.html');
					/*
					$submoduls = array mit strings
					$active = integer des gerade aktiven tabs von 0 bis 2
					$content = der html-inhalt*/
					#$LANG->getLL('submodule_commune');#'Allgemeines zur Kommune';	
/*					if (tx_itaoshhmanager_general::hasRights(4,2)) {
						$submoduls[2] = $LANG->getLL('submodule_school_user');
						if ($schoolid !=0 ) {
							$submoduls[2] = 'Zugangsdaten anlegen';
						}
...
*/					
					if (tx_itaoshhmanager_general::hasRights(5,2)) {
						...
						if ($schoolid !=0 ) {
							$submoduls[5] = 'Administrator Schule anlegen';
						}
					}
					$submoduls[0] = "Schule ausw&auml;hlen";
					$active = 0;
...
#								if (tx_itaoshhmanager_general::isInBeUserGroup($BE_USER->user['usergroup'], $this->tsvars['be_groupid_redadmin_kommune']) &&  !$BE_USER->user['admin']) {
									$linkerg = '&beuser_school=1';
#								}
								$markerArray_td['###SCHOOL_NAME###'] = '<a href="'.$this->indexpath.'&smnr=25&communeid='.$communeid.'&schoolid='.$val['uid'].$linkerg.'" title="zu den Details der Schule">';
							...
							$markerArray_td['###CLASS_DISPLAY###'] = (tx_itaoshhmanager_general::hasRights(2,3)) ? '' : 'display_none';
							$html.= tx_itaoshhmanager_library::template_ausgeben($markerArray_td, 'TEMPLATE_TABLEROWS_SCHOOLS', 'layout/be_template_mod2.html');
						}
					} else {
						$html = utf8_encode(tx_itaoshhmanager_library::template_ausgeben($markerArray_td, 'TEMPLATE_NO_SCHOOLS_TABLE', 'layout/be_template_mod2.html'));
					}
					return $html;
				}
				...
				function getSchoolDetailsPage($schoolid) {
					...
					if ($schoolid > 0) {
						tx_itaoshhmanager_database::checkAnzResults($schoolid);
					}
					
					if ($this->piVars['switchstatus']) {
						$switched = tx_itaoshhmanager_database::switchPhase($this->piVars['neuePhase'],$schoolid);
					}
					if ($_GET['switchback']) {
						$switched = tx_itaoshhmanager_database::switchPhaseBack($this->piVars['switchback'],$schoolid);
					}
					...
					$ma_det_sc['###BOX_ABSTIMMUNG###'] = $this->getBoxBaseAbstimmung($schoolid);
					$ma_det_sc['###BOX_ERGEBNIS###'] = $this->getBoxBaseErgebnis($schoolid);
					$ma_det_sc['###SIDEBOX###'] = "";#$this->getSidebox($schoolid);
					
					$html = tx_itaoshhmanager_library::template_ausgeben($ma_det_sc, 'TEMPLATE_SCHOOL_DETAIL_CONTENT', 'layout/be_template_mod2.html');	
					...
					$html_oben =  tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPLATE_SCHOOL_DETAILPAGE', 'layout/be_template_mod2.html');
					/*
					$submoduls = array mit strings
					$active = integer des gerade aktiven tabs von 0 bis 2
					$content = der html-inhalt*/
					...
					if (tx_itaoshhmanager_general::hasRights(1,1)) { # wenn jemand die Komune ansehen darf
						$submoduls[0] = "Schule ausw&auml;hlen";#$LANG->getLL('submodule_commune');#
						$xmod++;
					}
					
					$this->myModule = "mod2";
					$submoduls[1] = "Aktueller Status/Phase";#'Phasen der Schule';#$LANG->getLL('submodule_school');
...
*/					
				...
				function getBoxBaseVorbereitung($schoolid) {
					global $LANG;
					$schooldata = tx_itaoshhmanager_database::getSchoolById($schoolid);
					$allPhasenSchool = tx_itaoshhmanager_database::getAllPhasen_sorted();
					$schulstatus = $schooldata['ref_status'];
					$markerArray['###BOXTOPIC###'] = 'vorbereitung';
					$markerArray['###BOXHEADER###'] = $allPhasenSchool[1]['title']; #$LANG->getLL('l_vorbereitung');	 #'Vorbereitung';
					$markerArray['###ACTIVITY###'] = ($schulstatus==1) ? '' : 'inaktivschl';
					$markerArray['###BOXCONTENT_CLASS###'] = ($schulstatus==1) ? 'boxcontent' : 'content_zu';					
					$markerArray['###URLSCHWITCHBACK###'] = $this->indexpath.'&smnr=1&communeid='.$schooldata['ref_commune'].'&schoolid='.$schoolid.'&switchback=1';
					$markerArray['###IMAGEPATH###'] = $this->imagepath2;
					$markerArray['###FORMURL###'] = $this->indexpath.'&smnr=1&communeid='.$schooldata['ref_commune'].'&schoolid='.$schoolid.'';
					
					
					$markerArray['###EDITHEADER###'] = ($schulstatus > 1) ? tx_itaoshhmanager_library::template_ausgeben($markerArray, 'SWITCHBACK', 'layout/be_template_mod2.html'): '';	#Switch back	
					...
					$markerArray['###BOXBODY###'] = $this->getBoxBodyVorbereitung($schoolid,$schooldata,$schulstatus);
					
					$html = tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_BASIC', 'layout/be_template_mod2.html');
					
					return $html;
				}
				
				function getBoxBodyVorbereitung($schoolid,$schooldata,$schulstatus) {
					$akt_status = 1;
					# @TODO: hier noch die Conditions prüfen, ob alles erfüllt ist, damit umgeschaltet werden kann
					$conditionsOkay = 1;
					$markerArray['###SCHALTE_AKTIV###'] = ($schulstatus == $akt_status && $conditionsOkay) ? '' : ' disabled="disabled" ';					
					$markerArray['###FORMURL###'] = $this->indexpath.'&smnr=1&communeid='.$schooldata['ref_commune'].'&schoolid='.$schoolid.'';
					
					$html = tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_BODY_VORBEREITUNG', 'layout/be_template_mod2.html');
					#$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_JS', 'layout/be_template_mod2.html');
					return $html;
				}
				...
				function getBoxBaseOnline($schoolid) {
					...
					$markerArray['###BOXHEADER###'] = $allPhasenSchool[2]['title']; #$LANG->getLL('l_online');	 #'Vorbereitung';
					$markerArray['###ACTIVITY###'] = ($schulstatus==2) ? '' : 'inaktivschl';
					$markerArray['###BOXCONTENT_CLASS###'] = ($schulstatus==2) ? 'boxcontent' : 'content_zu';					
					$markerArray['###URLSCHWITCHBACK###'] = $this->indexpath.'&smnr=1&communeid='.$schooldata['ref_commune'].'&schoolid='.$schoolid.'&switchback=2';
					$markerArray['###IMAGEPATH###'] = $this->imagepath2;
					$markerArray['###FORMURL###'] = $this->indexpath.'&smnr=1&communeid='.$schooldata['ref_commune'].'&schoolid='.$schoolid.'';
					
					$markerArray['###EDITHEADER###'] = ($schulstatus > 2) ? tx_itaoshhmanager_library::template_ausgeben($markerArray, 'SWITCHBACK', 'layout/be_template_mod2.html'): '';	#Switch back		
					...
					#$disable_stoppen = ($schulstatus == 1) ? '' : ' disabled="disabled" ';
					$markerArray['###BOXBODY###'] = $this->getBoxBodyOnline($schoolid,$schooldata,$schulstatus);
				...
				function getBoxBodyOnline($schoolid,$schooldata,$schulstatus) {
					$akt_status = 2;
					# @TODO: hier noch die Conditions prüfen, ob alles erfüllt ist, damit umgeschaltet werden kann
					$conditionsOkay = 1;
					$markerArray['###SCHALTE_AKTIV###'] = ($schulstatus == $akt_status && $conditionsOkay) ? '' : ' disabled="disabled" ';					
					$markerArray['###FORMURL###'] = $this->indexpath.'&smnr=1&communeid='.$schooldata['ref_commune'].'&schoolid='.$schoolid.'';
					$html = tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_BODY_ONLINE', 'layout/be_template_mod2.html');
					#$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_JS', 'layout/be_template_mod2.html');
					return $html;
				}
				...
				function getBoxBaseAbstimmung($schoolid) {
					global $LANG;
					$schooldata = tx_itaoshhmanager_database::getSchoolById($schoolid);
					$allPhasenSchool = tx_itaoshhmanager_database::getAllPhasen();
					$schulstatus = $schooldata['ref_status'];
					$markerArray['###BOXTOPIC###'] = 'abstimmung';
					$markerArray['###BOXHEADER###'] = $allPhasenSchool[3]['title']; #$LANG->getLL('l_abstimmung');	 #'Vorbereitung';
					$markerArray['###ACTIVITY###'] = ($schulstatus==3) ? '' : 'inaktivschl';
					$markerArray['###BOXCONTENT_CLASS###'] = ($schulstatus==3) ? 'boxcontent' : 'content_zu';					
					$markerArray['###URLSCHWITCHBACK###'] = $this->indexpath.'&smnr=1&communeid='.$schooldata['ref_commune'].'&schoolid='.$schoolid.'&switchback=3';
					$markerArray['###IMAGEPATH###'] = $this->imagepath2;
					$markerArray['###FORMURL###'] = $this->indexpath.'&smnr=1&communeid='.$schooldata['ref_commune'].'&schoolid='.$schoolid.'';
					
					$markerArray['###EDITHEADER###'] = ($schulstatus > 3) ? tx_itaoshhmanager_library::template_ausgeben($markerArray, 'SWITCHBACK', 'layout/be_template_mod2.html'): '';	#Switch back		
					...
					#$disable_stoppen = ($schulstatus == 1) ? '' : ' disabled="disabled" ';
					$markerArray['###BOXBODY###'] = $this->getBoxBodyAbstimmung($schoolid,$schooldata,$schulstatus);
				...
				function getBoxBodyAbstimmung($schoolid,$schooldata,$schulstatus) {
					global $LANG;
					$communeid = $schooldata['ref_commune'];
					$akt_status = 3;
					# @TODO: hier noch die Conditions prüfen, ob alles erfüllt ist, damit umgeschaltet werden kann
					...
					$markerArray['###SCHALTE_AKTIV###'] = ($schulstatus == $akt_status && $conditionsOkay) ? '' : ' disabled="disabled" ';					
					$markerArray['###FORMURL###'] = $this->indexpath.'&smnr=1&communeid='.$schooldata['ref_commune'].'&schoolid='.$schoolid.'';		
					$markerArray['###LINKID_RESULTPAGE###'] = $schooldata['resultpage'];
					
					# neu für anzahl der Results, die im Plugin angezeigt werden sollen:
					$markerArray['###ANZAHL_RESULTS###'] = $schooldata['number_of_result_offers'];
					
					$back_params = array("smnr" => $this->piVars['smnr'], "communeid" => $communeid, "schoolid" => $schoolid);		
					$resanzid = 	tx_itaoshhmanager_database::getresanzid($schoolid);				
					####$thelink = str_replace("]=edit","]=edit&amp;columnsOnly=number_of_result_offers",tx_itaoshhmanager_general::getEditLink($LANG->getLL('l_editanzresults'), 'tx_itaoshhmanager_schools', $schoolid,array(), $back_params,1,$LANG->getLL('l_editanzresults'),1));
					$thelink = str_replace("]=edit","]=edit&amp;columnsOnly=number_of_result_offers",tx_itaoshhmanager_general::getEditLink($LANG->getLL('l_editanzresults'), 'tx_itaoshhmanager_resultanz', $resanzid,array(), $back_params,1,$LANG->getLL('l_editanzresults'),1));
					
					$markerArray['###CHANGE_ANZRESULTS_LINK###'] = $thelink;	
					$markerArray['###CLASS_ANZRES###'] = ($schooldata['ref_status'] == 3) ? '' : 'display_none';
					#				/typo3/mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM3&smnr=3&communeid=7&schoolid=9&vorschlaege=1&edit_all_stimmen=1
					$markerArray['###LINK_STIMMENEINGABE###'] = "/typo3/mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM3";
					$markerArray['###LINK_STIMMENEINGABE###'] .= '&smnr=3communeid='.$communeid.'&schoolid='.$schoolid.'&vorschlaege=1&edit_all_stimmen=1';
					
					$html = tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_BODY_ABSTIMMUNG', 'layout/be_template_mod2.html');
					$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_JS', 'layout/be_template_mod2.html');
					return $html;
				}
				...
				function getBoxBaseErgebnis($schoolid) {
					...
					$markerArray['###BOXHEADER###'] = $allPhasenSchool[5]['title']; #$LANG->getLL('l_ergebnis');	 #'Vorbereitung';
					$markerArray['###ACTIVITY###'] = ($schulstatus==5) ? '' : 'inaktivschl';
					$markerArray['###BOXCONTENT_CLASS###'] = ($schulstatus==5) ? 'boxcontent' : 'content_zu';					
					$markerArray['###EDITHEADER###'] = '';		
							
					$markerArray['###STATUS_BEZ###'] = ($schulstatus < 5) ? $LANG->getLL('l_status_notstarted'):  $LANG->getLL('l_status_running');	
					$markerArray['###STATUS_BEZ###'] = ($schulstatus > 5) ? $LANG->getLL('l_status_finished'):  $markerArray['###STATUS_BEZ###'];									
					$markerArray['###STATUS_ERH_CLASS###'] = ($schulstatus < 5) ? 'actstateclass_1' : 'actstateclass_2';									
					$markerArray['###STATUS_ERH_CLASS###'] = ($schulstatus > 5) ? 'actstateclass_4' : $markerArray['###STATUS_ERH_CLASS###'];			
					#$disable_stoppen = ($schulstatus == 1) ? '' : ' disabled="disabled" ';
					$markerArray['###BOXBODY###'] = $this->getBoxBodyErgebnis($schoolid,$schooldata,$schulstatus);
				...
				function getBoxBodyErgebnis($schoolid,$schooldata,$schulstatus) {
					global $LANG;
					$communeid = $schooldata['ref_commune'];
					$akt_status = 5;
					# @TODO: hier noch die Conditions prüfen, ob alles erfüllt ist, damit umgeschaltet werden kann
					...
					$thelink = str_replace("]=edit","]=edit&amp;columnsOnly=number_of_result_offers",tx_itaoshhmanager_general::getEditLink($LANG->getLL('l_editanzresults'), 'tx_itaoshhmanager_resultanz', $resanzid,array(), $back_params,1,$LANG->getLL('l_editanzresults'),1));
					$markerArray['###CHANGE_ANZRESULTS_LINK###'] = $thelink;	
					$markerArray['###CLASS_ANZRES###'] = ($schooldata['ref_status'] >= 4) ? '' : 'display_none';
					$markerArray['###LINKID_RESULTPAGE###'] = $schooldata['resultpage'];
					
					
					$html = tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_BODY_ERGEBNIS', 'layout/be_template_mod2.html');
					$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_JS', 'layout/be_template_mod2.html');
					return $html;
				}
				...
				function getSidebox($schoolid) {
					...
					$html .= tx_itaoshhmanager_library::template_ausgeben($ma_det_sc, 'SIDEBOX_SCHOOL', 'layout/be_template_mod2.html');
					return $html;
				}
				...
				function getBenutzerPage($schoolid) {
					...
					$locat = urlencode('mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM2');
					$ma['###JS_EDIT###'] = tx_itaoshhmanager_library::js_fuer_ds_bearbeiten($locat);
					$html_oben =  tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPLATE_USER_DETAILPAGE', 'layout/be_template_mod2.html');
					...
					$html = tx_itaoshhmanager_library::template_ausgeben($ma_det_sc, 'TEMPLATE_USER_DETAIL_CONTENT', 'layout/be_template_mod2.html');	
...
*/					
				...
				function getBoxBodyVerwKommune($schoolid,$groupid,$communeid2 = 0) {
					...
					$ma['###GROUPID###'] = $groupid;
					$markerArray['###GENERATE_ACCOUNTS###'] = tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPL_GENERATE_ACCOUNTS', 'layout/be_template_mod2.html');									
					...
					if (is_file($_SERVER['DOCUMENT_ROOT']."/".$this->tsvars['accountpath'].$pdfdateiname)) {
						...
						$markerArray['###DOWNLOAD_ACCOUNTS###'] = tx_itaoshhmanager_library::template_ausgeben($ma_dl,'TEMP_DOWNLOAD_ACCOUNTS','layout/be_template_mod2.html');
						
						$ma_upd['###PFAD_UPDATE_FEACCOUNTS###'] = $this->indexpath.'&communeid='.$communeid.'&schoolid='.$schoolid.'&benutzer=1&anz_acc_upd_'.$groupid.'=1';
						$markerArray['###UPDATE_ACCOUNTS###'] = tx_itaoshhmanager_library::template_ausgeben($ma_upd,'TEMP_UPDATE_ACCOUNTS','layout/be_template_mod2.html');
					} else {						
						$markerArray['###DOWNLOAD_ACCOUNTS###'] = "";#" [noch kein PDF vorhanden]";
						$markerArray['###UPDATE_ACCOUNTS###'] = "";
					}
					
					$html = tx_itaoshhmanager_library::template_ausgeben($markerArray,$templPart , 'layout/be_template_mod2.html');
					#$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'BOX_JS', 'layout/be_template_mod2.html');
					return $html;
				}
				...
				function getSideboxUser($schoolid) {
					...
					$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'SIDEBOX_USER', 'layout/be_template_mod2.html');
					return $html;
				}
...
//							$sortArr[6]['title'] = 'help';				
					...
					# Content über Tabs:
					$ma['###SCHOOL_TITLE###'] = $schooldata['title'];
					$ma['###SC_ORT###'] = ($schooldata['city']!="") ? '('.$schooldata['city'].')' : '';$locat = urlencode('mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM2');
					$ma['###JS_EDIT###'] = tx_itaoshhmanager_library::js_fuer_ds_bearbeiten($locat);
					$html_oben =  tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPLATE_OFFERS_DETAILPAGE', 'layout/be_template_mod2.html');
					...
					$ma_det_sc['###ANZ_ERGEBNISSE###'] = $this->anz_ergebnisse;
						...
						$ma_det_sc['###LINK_DOC_GEN###'] = tx_itaoshhmanager_library::template_ausgeben($ma_doc, 'T_GENERATE_DOC', 'layout/be_template_mod2.html');	
					...
					$html = tx_itaoshhmanager_library::template_ausgeben($ma_det_sc, 'TEMPLATE_OFFERS_DETAIL_CONTENT', 'layout/be_template_mod2.html');	
...
#!#!#						$submoduls[8] = $LANG->getLL('submodule_school_editfeuser');
				...
				function getSideboxOffers($schoolid) {
					...
					$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'SIDEBOX_OFFERS', 'layout/be_template_mod2.html');
					return $html;
				}
...
//							$markerArray['###HELP_WANTED###'] 	=  ($val['costs_help']) ? 'ja' : 'nein';
							...
							$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'TEMPLATE_TABLEROWS_OFFERS', 'layout/be_template_mod2.html');
						}
					} else {
						$html .=tx_itaoshhmanager_library::template_ausgeben($markerArray, 'TEMPLATE_NO_OFFERS', 'layout/be_template_mod2.html');
					}
				...
				function getOfferDetailPage($schoolid, $offerid) {
					...
					$ma['###SC_ORT###'] = ($schooldata['city']!="") ? '('.$schooldata['city'].')' : '';$locat = urlencode('mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM2');
					$ma['###JS_EDIT###'] = tx_itaoshhmanager_library::js_fuer_ds_bearbeiten($locat);
					$html_oben =  tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPLATE_OFFER_VIEW', 'layout/be_template_mod2.html');
					...
					#$ma_off_det['###FUNKTIONSLEISTE###'] = ($offerData['status'] > 1) ? '' : tx_itaoshhmanager_library::template_ausgeben($off_funcs, 'OFFER_FUNCTIONS', 'layout/be_template_mod2.html');
					...
					$ma_off_det['###FUNKTIONSLEISTE###'] = tx_itaoshhmanager_library::template_ausgeben($off_funcs, 'OFFER_FUNCTIONS', 'layout/be_template_mod2.html');
...
//					$ma_off_det['###OFF_HELP###'] = ($offerData['costs_help']) ? '' : 'keine ';				
					...
					$html = tx_itaoshhmanager_library::template_ausgeben($ma_off_det, 'TEMPLATE_OFFER_VIEW_CONTENT', 'layout/be_template_mod2.html');	
...
#!#!#						$submoduls[8] = $LANG->getLL('submodule_school_editfeuser');
				...
				function getSideboxOffersDetail($schoolid, $offerid) {
					...
					$html .= tx_itaoshhmanager_library::template_ausgeben($ma_off_det, 'SIDEBOX_OFFER_DETAIL', 'layout/be_template_mod2.html');
					return $html;
				}
				...
				function getBeUserPage($communeid, $schoolid) {					
					...
					$locat = urlencode('mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM2');
					$ma['###JS_EDIT###'] = tx_itaoshhmanager_library::js_fuer_ds_bearbeiten($locat);
					$html_oben =  tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPLATE_BEUSER_DETAILPAGE', 'layout/be_template_mod2.html');
					...
					$html = tx_itaoshhmanager_library::template_ausgeben($ma_det_sc, 'TEMPLATE_BEUSER_DETAIL_CONTENT', 'layout/be_template_mod2.html');	
...
#						$submoduls[8] = $LANG->getLL('submodule_school_editfeuser');
				  ...
				  * Hier wird eine Oberfläche generiert, auf der man für die ausgewählte Schule BE-Redakteure anlegen kann - Nur für technischen Admin!
				  */
				function getBeUserPage_school($communeid, $schoolid) {					
					global $LANG, $BE_USER;
					$GLOBALS["BE_USER"]->setAndSaveSessionData ('search_value', "");
					#t3lib_utility_debug::debug(tx_itaoshhmanager_library::my_hashPassword("abcdefg"));
					$schooldata = tx_itaoshhmanager_database::getSchoolById($schoolid);
...
*/		
					if (tx_itaoshhmanager_general::hasRights(5,2) || tx_itaoshhmanager_general::hasRights(6,2) ) {
						if (!tx_itaoshhmanager_general::isInBeUserGroup($BE_USER->user['usergroup'], $this->tsvars['be_groupid_redadmin_schule']) || $BE_USER->user['admin']) {
							$submoduls[5] = $LANG->getLL('submodule_school_beuser');
						}
						if ($schoolid !=0 ) {
							#$submoduls[5] = 'Administrator Schule anlegen';
					...
					$submoduls[25] = 'Administrator Schule anlegen';
...
#						$submoduls[8] = $LANG->getLL('submodule_school_editfeuser');
				...
				function getBoxBodyBeUser($communeid,$schoolid,$groupid) {
					...
					$ma['###GROUPID###'] = $groupid;
					$markerArray['###GENERATE_ACCOUNTS###'] = tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPL_GENERATE_BEACCOUNTS', 'layout/be_template_mod2.html');	
					if ($this->piVars['beuser_school']) {
						$param_ende = '&beuser_school=1';
					} else {
						$param_ende = '&beuser=1';
					}								 
					$markerArray['###FORM_ACTION_GENACC###'] = $this->indexpath.'&communeid='.$communeid.'&schoolid='.$schoolid.$param_ende;#'&beuser=1';
					...
					$pdfdateiname_link = "red_zugangsdaten_".$pref[$groupid]."_".$uid_sk."-".$groupid.".pdf?".time();
					#$pdfdateiname = "zugangsdaten_".$schoolid."-".$groupid.".pdf";	
					if (is_file($_SERVER['DOCUMENT_ROOT']."/".$this->tsvars['beaccountpath'].$pdfdateiname)) {
						$ma_dl['###PFAD_DOWNLOAD###'] = 'http://'.$_SERVER['HTTP_HOST']."/".$this->tsvars['beaccountpath'].$pdfdateiname_link;
						$markerArray['###DOWNLOAD_ACCOUNTS###'] = tx_itaoshhmanager_library::template_ausgeben($ma_dl,'TEMP_DOWNLOAD_BEACCOUNTS','layout/be_template_mod2.html');
						
						$ma_upd['###PFAD_UPDATE_BEACCOUNTS###'] = $this->indexpath.'&communeid='.$communeid.'&schoolid='.$schoolid.'&beuser=1&anz_beacc_upd_'.$groupid.'=1';
						$markerArray['###UPDATE_ACCOUNTS###'] = tx_itaoshhmanager_library::template_ausgeben($ma_upd,'TEMP_UPDATE_BEACCOUNTS','layout/be_template_mod2.html');
					} else {						
						$markerArray['###DOWNLOAD_ACCOUNTS###'] = "";#" [noch kein PDF vorhanden]";
						$markerArray['###UPDATE_ACCOUNTS###'] = "";
					}
				...
				function getBenutzerEditPage($communeid,$schoolid) {
					...
					$ma['###SC_ORT###'] = ($schooldata['city']!="") ? '('.$schooldata['city'].')' : '';$locat = urlencode('mod.php?M=txitaoshhmanagerM0_txitaoshhmanagerM2');
					$ma['###JS_EDIT###'] = tx_itaoshhmanager_library::js_fuer_ds_bearbeiten($locat);
					$html_oben =  tx_itaoshhmanager_library::template_ausgeben($ma, 'TEMPLATE_FEUSER_EDITPAGE', 'layout/be_template_mod2.html');
				...
				#			$search_value_group = $GLOBALS["BE_USER"]->getSessionData("filter_usergroups");
					...
					$html = tx_itaoshhmanager_library::template_ausgeben($ma_det_sc, 'TEMPLATE_FEUSER_EDIT_CONTENT', 'layout/be_template_mod2.html');	
					...
					if (is_array($allOffers)) {
						...
						while (list ($key, $val) = each ($allOffers)) {
							...
							*/
							...
							$html .= tx_itaoshhmanager_library::template_ausgeben($markerArray, 'TEMPLATE_TABLEROWS_USERSEDIT', 'layout/be_template_mod2.html');
						}
					} else {
						$html .=tx_itaoshhmanager_library::template_ausgeben($markerArray, 'TEMPLATE_NO_USERSEDIT', 'layout/be_template_mod2.html');
					}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/itao_shh_manager/mod2/index.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/itao_shh_manager/mod2/index.php']);
}
...
// Make instance:
$SOBE = t3lib_div::makeInstance('tx_itaoshhmanager_module2');
$SOBE->init();
