...
class tx_datamintsfeuser_pi1 extends tslib_pibase {
	...
	function sendActivationMail($userId) {
		...
		$bestaetigLink = $extraMarkers['approvallink'];
		$zurueckLink = $extraMarkers['disapprovallink'];
		$extraMarkers['approvallink'] = '<a href="'.$bestaetigLink.'">'.$this->pi_getLL('at_best').'</a>'; #Antrag best&auml;tigen
		$extraMarkers['disapprovallink'] = '<a href="'.$zurueckLink.'">'.$this->pi_getLL('at_zurueck').'</a>';
		

		// E-Mail senden.
		$this->conf['register.']['mailtype'] = 'html';
	...
	function makeApprovalCheck($userId) {
		...
		if ($this->piVars[$this->contentId]['hash'] == $hashApproval && $row['tx_datamintsfeuser_approval_level'] == 1) {
			...
			// User aktivieren.
			$GLOBALS['TYPO3_DB']->exec_UPDATEquery('fe_users', 'uid = ' . $userId ,array('tstamp' => $time, 'disable' => '0', 'usergroup' => '2', 'tx_datamintsfeuser_approval_level' => '0'));
